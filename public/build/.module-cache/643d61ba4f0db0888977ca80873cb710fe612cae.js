...
var data = [
	{fieldName: "MaxLength", fieldValue: 50},
	{fieldName: "BackupDirectory", fieldValue: "/tmp"},
	{fieldName: "ShouldForce", fieldValue: true}
];
...
var ConfigComponentsList = React.createClass({displayName: "ConfigComponentsList",
  ...
  handleValChange: function(changedComp) {
  	var newData = this.props.data.forEach(function(comp) {
  		if (comp.fieldName == changedComp.props.fieldName) {
  			comp.fieldValue = changedComp.state.value;
  		}
  	});
  ...
  renderComponents: function() {
  	...
  	return this.state.data.map(function (compData, index) {
  		return (React.createElement(ConfigComponent, {key: index, onChange: this.handleValChange, fieldName: compData.fieldName, fieldType: compData.fieldType, value: compData.fieldValue}))
  	});
  },
