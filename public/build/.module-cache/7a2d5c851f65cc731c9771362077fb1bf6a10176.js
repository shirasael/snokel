...
function getInputElement(elmType, ref, val) {
	if (elmType == "num") {
		return ( React.createElement(NumberInput, {ref: ref, val: val}) );
	} else if (elmType == "str") {
		return ( React.createElement(StringInput, {ref: ref, val: val}) );
	} else if (elmType == "bool") {
		return ( React.createElement(BoolInput, {ref: ref, val: val}) );
	} else {
		return null;
	}
};
...
var NumberInput = React.createClass({displayName: "NumberInput",
	render: function() {
		return (
			React.createElement("input", {type: "number", value: this.props.val})
		);
	}
});
...
var StringInput = React.createClass({displayName: "StringInput",
	render: function() {
		return (
			React.createElement("input", {type: "text", value: this.props.val})
		);
	}
});
...
var BoolInput = React.createClass({displayName: "BoolInput",
	render: function() {
		return (
			React.createElement("input", {type: "checkbox", checked: this.props.val})
		);
	}
});
