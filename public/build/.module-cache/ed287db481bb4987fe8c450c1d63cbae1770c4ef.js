...
var ConfigComponentsList = React.createClass({displayName: "ConfigComponentsList",
  ...
  handleValChange: function(comp) {
  	this.state.data[comp.props.fieldName] = comp.state.value;
  	var newMeta = this.state.dataMeta;
  	if (comp.state.dataMeta && this.state.dataMeta != comp.state.dataMeta) {
  		newMeta = comp.state.dataMeta;
  	}
  ...
  renderComponents: function() {
  	...
  	for (compName in this.state.data) {
  		var type = this.state.dataMeta[compName].self;
  		var val = this.state.data[compName];
  		comps.push(React.createElement(ConfigComponent, {key: index, onChange: this.handleValChange, fieldName: compName, fieldType: type, value: val, dataMeta: this.state.dataMeta}))
  		index++;
  	}
  	return comps;
  },
