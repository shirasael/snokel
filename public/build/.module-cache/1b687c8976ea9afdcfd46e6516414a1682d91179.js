...
var ConfigComponentsList = React.createClass({displayName: "ConfigComponentsList",
  ...
  handleValChange: function(comp) {
  	...
  	this.state.dataMeta[comp.props.fieldName] = comp.state.dataMeta;
  	this.setState({data : this.state.data, dataMeta : newMeta});
  },
  renderComponents: function() {
  	...
  	for (compName in this.state.data) {
  		...
  		comps.push(React.createElement(ConfigComponent, {key: index, onChange: this.handleValChange, fieldName: compName, value: val, dataMeta: meta}))
  		index++;
  	}
  	return comps;
  },
