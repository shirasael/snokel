...
var ListInput = React.createClass({displayName: "ListInput",
	...
	renderInnerComponents: function() {
		...
		for (var index = 0; index < this.state.value.length; index++) {
			...
			var comp = InputGenerator.generateInputElement(val, this.handleChange, this.state.dataMeta[index], index);
			comps.push(
...
var InputGenerator = {
	generateInputElement: function(val, onChange, meta, ref) {
		var elmType = meta.self;
		if (elmType == "num") {
			return ( React.createElement(NumberInput, {value: val, valueChanged: onChange, dataMeta: meta, ref: ref}) );
		} else if (elmType == "str") {
			return ( React.createElement(StringInput, {value: val, valueChanged: onChange, dataMeta: meta, ref: ref}) );
		} else if (elmType == "bool") {
			return ( React.createElement(BoolInput, {value: val, valueChanged: onChange, dataMeta: meta, ref: ref}) );
		} else if (elmType == "list") {
			return ( React.createElement(ListInput, {val: val, valueChanged: onChange, dataMeta: meta, ref: ref}) );
		} else {
			return null;
		}
	}
