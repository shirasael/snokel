	...
	renderInnerComponents: function() {
		...
		for (innerComp in this.state.value) {
			for (innerCompAtt in innerComp) {
	  		...
	  		comps.push(
	  			React.createElement(ConfigComponent, {ref: index, onChange: this.handleValChange, fieldName: innerCompAtt, fieldType: type, value: val})
	  		);
			}
  		index++;
		}
...
var ConfigComponent = React.createClass({displayName: "ConfigComponent",
	...
	getInputElement: function(ref) {
		...
		if (elmType == "num") {
			...
			return ( React.createElement(ListInput, {ref: ref, val: val, valueChanged: this.handleChange, dataMeta: this.props.dataMeta}) );
		} else {
			return null;
		}
	},
